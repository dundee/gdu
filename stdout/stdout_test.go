...
func TestFormatSizeFixedUnitBinary(t *testing.T) {
	...
	ui := CreateStdoutUI(output, false, false, false, false, false, false, false, false, "k", 0, false)
	assert.Equal(t, "1.0 KiB", ui.formatSize(1024))
	assert.Equal(t, "1.5 KiB", ui.formatSize(1536))

	ui = CreateStdoutUI(output, false, false, false, false, false, false, false, false, "m", 0, false)
	assert.Equal(t, "1.0 MiB", ui.formatSize(1024*1024))
	assert.Equal(t, "1.5 MiB", ui.formatSize(1536*1024))

	ui = CreateStdoutUI(output, false, false, false, false, false, false, false, false, "g", 0, false)
	assert.Equal(t, "1.0 GiB", ui.formatSize(1024*1024*1024))
	assert.Equal(t, "1.5 GiB", ui.formatSize(1536*1024*1024))
}
func TestFormatSizeFixedUnitSI(t *testing.T) {
	output := bytes.NewBuffer(make([]byte, 10))

	// -k --si
	ui := CreateStdoutUI(output, false, false, false, false, false, false, true, false, "k", 0, false)
	assert.Equal(t, "1.0 kB", ui.formatSize(1000))
	assert.Equal(t, "1.5 kB", ui.formatSize(1500))

	ui = CreateStdoutUI(output, false, false, false, false, false, false, true, false, "m", 0, false)
	assert.Equal(t, "1.0 MB", ui.formatSize(1000000))
	assert.Equal(t, "1.5 MB", ui.formatSize(1.5e+6))

	ui = CreateStdoutUI(output, false, false, false, false, false, false, true, false, "g", 0, false)
	assert.Equal(t, "1.0 GB", ui.formatSize(1e+9))
	assert.Equal(t, "1.5 GB", ui.formatSize(1.5e+9))
}
